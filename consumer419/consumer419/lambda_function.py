...
def create_entities(row):
    """Uses AWS Comprehend to Create Entities on a DataFrame"""
...
def apply_entities(df, column="wikipedia_snippit"):
    ...
    df["Entitites"] = df[column].apply(create_entities)
    return df
...
def lambda_handler(event, context):
    ...
    LOG.info(f"Entity from FANG companies: {df.to_dict()}")

    # Write result to S3
    write_s3(df=df, bucket="fangsentiment419", name=names)